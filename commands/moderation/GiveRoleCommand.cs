        ...
        public override async Task onCommand(SocketSlashCommand command)
        {
            ...
            { 
                await command.ModifyOriginalResponseAsync(x => {
                    x.Content = "Невозможно изменить роль великому Альтрону!";
                });
            }
            if (!(bool)options.ElementAtOrDefault(2))
            {
                try
                {
                    ...
                    await command.ModifyOriginalResponseAsync(x => {
                        x.Content = "Успешно выдали роль " + MentionUtils.MentionRole(((IRole)options[1].Value).Id) + " участнику " + MentionUtils.MentionUser(((IUser)options[0].Value).Id);
                    });
                }
                catch (Exception e)
                {
                    await command.ModifyOriginalResponseAsync(x => {
                        x.Content = "Не удалось выдать роль " + MentionUtils.MentionRole(((IRole)options[1].Value).Id) + " участнику " + MentionUtils.MentionUser(((IUser)options[0].Value).Id);
                    });
                }
            }
            else
            {
                try
                {
                    ...
                    await command.ModifyOriginalResponseAsync(x => {
                        x.Content = "Успешно забрали роль " + MentionUtils.MentionRole(((IRole)options[1].Value).Id) + " с участника " + MentionUtils.MentionUser(((IUser)options[0].Value).Id);
                    });
                }
                catch (Exception e)
                {
                    await command.ModifyOriginalResponseAsync(x => {
                        x.Content = "Не удалось забрать роль " + MentionUtils.MentionRole(((IRole)options[1].Value).Id) + " у участника " + MentionUtils.MentionUser(((IUser)options[0].Value).Id);
                    });
                }
            }
        }
