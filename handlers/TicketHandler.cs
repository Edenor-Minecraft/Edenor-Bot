...
namespace Discord_Bot.handlers
{
    internal class TicketHandler
    {
       ...
       {
            if (thread.ParentChannel.Id == 1099416318140223588)
            {
                foreach (var user in thread.Users)
                {
                    if (user.Id == 710401785663193158)
                        return;
                }
                ...
                await thread.SendMessageAsync(null, embed: embed.Build(), components: new ComponentBuilder().WithButton(bttn).Build());
            }
            else
            {
                return;
